...
asks for a login or a verification check, the tool opens a real embedded
browser window and **you** sign in or complete the check yourself; it only
saves the cookies that session produces so the download can continue. It
...
- Python packages:
  ...
  ```
  pip install pyside6 yt-dlp gallery-dl requests pyqtdarktheme
  ```

  - `pyside6` includes `PySide6-Addons`, which provides QtWebEngine for
    the **Login & Capture** browser window. Without it the app still
    runs; only the sign-in window is unavailable.
...
Settings, logs, and cookies are kept in an `app_data` folder created next to
the script, or next to the `.exe` in a built version (see **Files it keeps** below). Downloads go to
...
- **Manage Cookies** — one row per site, and **Add Site** for any other.
  ...
  - **Login & Capture** opens a built-in browser where you sign in, then
    **Save Session & Close** saves that site's cookies.
  ...
  sign-in window opens with your saved cookies already loaded, so you stay
  signed in there between uses.
- Downloads that use a site's saved cookies present the same browser
  identity (User-Agent) as the sign-in window that created them. Some
  sites, TikTok in particular, reject a session that arrives from a
  different "browser".
- If a site refuses the built-in sign-in window itself (for example
  TikTok's "Access denied / HTTP ERROR 403", which comes and goes), the
  window says so and won't save that session by default. **Start Fresh**
  in that window clears its own cookies and cache for the site and reloads
...
- If a download fails with something that looks like a login wall, rate
  ...
  sign-in window for you, and retries once after you save the session.
...
had picked from elsewhere, and the sign-in window's old unencrypted
browser data. While protection is on, the sign-in window keeps nothing on
disk, and cookies are handed to yt-dlp directly in memory; gallery-dl
...
requests, pyqtdarktheme, and FFmpeg, and ticks everything that's out
...
- **yt-dlp, PySide6, requests and the dark theme** are loaded while the app
...
All inside `app_data` next to the script or `.exe` (**Open Logs Folder** opens it):
...
- `cookies\` — one file per site (`<site>.txt` when plain,
  ...
  their cookie files.
- `webengine_profiles\` — the sign-in window's browser data per site, used
...
See `BUILD_INSTRUCTIONS_original.txt`. Build in its own virtual
environment. The result is a folder (`dist\Modern Video Downloader`)
with the `.exe` inside; a rebuild replaces that whole folder, including
the `app_data` the app created there, so back it up first or run the app
from a copy elsewhere.

For a much smaller build that signs in through Edge or Chrome instead of
a built-in browser, see **Modern Video Downloader Lite**
(`video_downloader_lite.py`).
